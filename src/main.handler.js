 ...
 * @param responseTransormerCallback An optional user provided callback method that will transform the http response returned by the route handler to any other form before being returned to client. It should accept resource to be returned and url parameter value map to apply custom logic to transfor it into the other form the callback function will return.
 * @constructor
 */
var Handler = function (routes, routeHandlers, parameterMapper, responseTransormerCallback) {
    ...
    this.handle = function (request) {
        ...
        if (handlerLookup && handlerLookup.name) {
            ...
            if (requestHandler) {
                ...
                return requestHandler(handlerPayload, parameterMapper, responseTransormerCallback);
            }
        }
